    ...
    <div className="space-y-6">
      <Card className="rounded-[12px] border-slate-200/70 bg-white shadow-[0_1px_4px_rgba(15,23,42,0.06)]">
        <CardHeader className="flex flex-row items-center justify-between px-5 pb-4 pt-5">
          <CardTitle className="text-[28px] font-bold leading-none tracking-[-0.02em] text-slate-800">Course Management</CardTitle>
          <Button
            className="h-11 rounded-[8px] bg-blue-600 px-5 text-[13px] font-medium text-white shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:bg-blue-700 hover:shadow-md"
        ...
        <CardContent className="space-y-5 px-5 pb-5">
          <div className="flex flex-wrap items-center gap-2 rounded-[10px] border border-slate-200 bg-white p-4">
            ...
            />
            <Button
              className="h-10 rounded-[8px] bg-blue-600 px-4 text-white transition-all duration-200 hover:bg-blue-700"
              onClick={() => {
                const normalizedQuery = String(query ?? '').trim()
                setQuery(normalizedQuery)
                setPage(1)
            ...
            {/* <Button
              variant="outline"
              className="h-10 rounded-[8px] border-slate-300 bg-white px-4"
              onClick={resetForm}
            >
              Reset Form
            </Button> */}
          ...
          <Table className="overflow-hidden rounded-[10px] border border-slate-200 bg-white">
            ...
            <TableBody>
              {courses.length === 0 ? (
                <TableEmptyState colSpan={7} title="No courses found" message="Add a course to start managing this list." />
              ) : (
                courses.map((course) => (
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 pt-1 text-sm text-slate-600">
            ...
            <div className="flex items-center gap-2">
              <select className="h-10 rounded-[8px] border border-slate-300 bg-white px-2" value={pageSize} onChange={(e) => { setPageSize(Number(e.target.value)); setPage(1) }}>
                {[25, 50, 75, 100].map((size) => (
                  <option key={size} value={size}>{size}</option>
                ))}
              </select>
              <Button variant="outline" className="rounded-[8px]" onClick={() => setPage((p) => Math.max(1, p - 1))}>Prev</Button>
              <Button variant="outline" className="rounded-[8px]" onClick={() => setPage((p) => Math.min(Math.max(1, Math.ceil(total / pageSize)), p + 1))}>Next</Button>
            </div>
          </div>
        </CardContent>
      </Card>
        ...
        <DialogContent className="max-w-[500px] p-0">
          <Card className="border-none shadow-none">
            <DialogHeader className="px-5 pb-2 pt-5">
              <DialogTitle className="text-[26px] font-bold leading-none tracking-[-0.01em] text-slate-800">{editingId ? 'Edit Course' : 'Add Course'}</DialogTitle>
            </DialogHeader>
            <CardContent className="px-5 pb-5 pt-3">
              <form className="grid grid-cols-1 gap-2.5" onSubmit={submit}>
                <div><Label>Code</Label><Input className="border-slate-200/70" required value={form.code} onChange={(e) => setForm((prev) => ({ ...prev, code: e.target.value }))} /></div>
                <div><Label>Name</Label><Input className="border-slate-200/70" required value={form.name} onChange={(e) => setForm((prev) => ({ ...prev, name: e.target.value }))} /></div>
                <div><Label>Credit Hours</Label><Input className="border-slate-200/70" type="number" min={1} value={form.credit_hours} onChange={(e) => setForm((prev) => ({ ...prev, credit_hours: e.target.value }))} /></div>
                <div><Label>Semester</Label><Input className="border-slate-200/70" type="number" min={1} value={form.semester} onChange={(e) => setForm((prev) => ({ ...prev, semester: e.target.value }))} /></div>
                <div><Label>Sessions / Week</Label><Input className="border-slate-200/70" type="number" min={1} value={form.sessions_per_week} onChange={(e) => setForm((prev) => ({ ...prev, sessions_per_week: e.target.value }))} /></div>
                <div>
                  <Label>Department</Label>
                  <select className="mt-1 h-10 w-full rounded-md border border-slate-200/70 bg-white px-3" value={form.department_id} onChange={(e) => setForm((prev) => ({ ...prev, department_id: e.target.value }))}>
                ...
                <div className="flex justify-end gap-2 pt-2">
                  <Button type="button" className="h-10 rounded-[8px] bg-slate-600 px-5 text-white hover:bg-slate-700" onClick={resetForm}>Cancel</Button>
                  <Button type="submit" className="h-10 rounded-[8px] bg-blue-600 px-5 text-white hover:bg-blue-700">Save</Button>
                </div>
              </form>
            </CardContent>
          </Card>
      ...
      {selectedCourseId && (
        <Card className="rounded-[14px] border-slate-200/70 bg-white shadow-[0_1px_4px_rgba(15,23,42,0.06)]">
          <CardHeader className="px-5 pb-3 pt-5">
            <CardTitle className="text-[22px] font-bold leading-none tracking-[-0.01em] text-slate-800">Faculty Assignment</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3 px-5 pb-5">
            <div className="flex flex-wrap gap-2">
              <select className="h-10 min-w-64 rounded-[10px] border border-slate-200 bg-white px-3" value={selectedFacultyId} onChange={(e) => setSelectedFacultyId(e.target.value)}>
                <option value="">-- Select Faculty --</option>
                {faculty.map((f) => (
                  <option key={f.id} value={f.id}>{f.full_name} ({f.email})</option>
                ))}
              </select>
              <Button className="h-10 rounded-[10px] bg-cyan-600 px-5 text-white hover:bg-cyan-700" onClick={assignFaculty}>Assign</Button>
            </div>
            <Table className="overflow-hidden rounded-[10px] border border-slate-200">
              ...
              <TableBody>
                {assignedFaculty.length === 0 ? (
                  <TableEmptyState colSpan={3} title="No faculty assigned" message="Assign faculty members to this course from above." />
                ) : (
